...
- Codex OAuth credentials are **per user**, never instance-global. The path scheme lives in `app/services/codex_home.ts` (`resolveUserCodexHome(userId)` → `<base>/<userId>`); both `CodexOauthService` (writes/reads via the `codex-auth` routes, scoped by `auth.getUserOrFail().id`) and `CodexProcessManager` (agent runtime, via `CodexEngine` using `traceIdentity.distinctId`) must derive the home from that helper. The runtime must NOT seed a user home from the host's `~/.codex/auth.json`; a user only has a credential after completing the OAuth device flow.
